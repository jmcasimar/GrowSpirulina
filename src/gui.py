...
class GUI:
    def __init__(self, logger = None, serialControl = None):
        ...
        if actualDirectory.endswith('src'):
            ...
            self.paths = ['./GUI_Pics/minus.png',
                          './GUI_Pics/plus.png']
        else:
            self.filename = './src/config.csv'
            self.paths = ['./src/GUI_Pics/minus.png',
    ...
    def begin(self):
         ...
         col1 = sg.Column([
                      [sg.Text('Grow Spirulina', size=(44, 1), justification='center', font=("Helvetica 30 bold"), relief=sg.RELIEF_RIDGE, border_width=5)],
                      [sg.Text('_' * 150)]
               ])
    ...
    def run(self):
        try:
            ...
            if int(values['cycleTime']) != self.cycleTime:
                ...
                else: self.window['Actualizar Ciclo'].Update(button_color=('black', self.blue1), disabled=False)
            ...
            if int(values['evTime']) != self.evTime:
                ...
                if self.piso!='' and self.lado!='' and self.etapa!='' and self.solucion!='':
                    ...
                    else: self.window['Actualizar'].Update(button_color=('black', self.blue1), disabled=False)
